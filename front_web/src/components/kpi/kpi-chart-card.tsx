...
const PORTALIS_THEME = {
  ...
  palette: {
    fills:   ['#1E5B3C', '#085499', '#92720C', '#A01D65', '#5829A8', '#0F6E56', '#D97706', '#DC2626'],
    strokes: ['#1E5B3C', '#085499', '#92720C', '#A01D65', '#5829A8', '#0F6E56', '#D97706', '#DC2626'],
  },
  overrides: {
    common: {
...
export function KpiChartCard({ kpi, loading = false, fullHeight = false }: KpiChartCardProps) {
  ...
  return (
    ...
    >
      ...
      <div className="px-4 pt-4 pb-3 border-b border-[#F0F4F8]">
        ...
        {kpi.description && (
          <div className="text-[11px] text-[#7691A8] mt-0.5 truncate" title={kpi.description}>
            {kpi.description}
          </div>
        )}
      </div>
      ...
      <div className="flex-1 px-2 py-2" style={{ minHeight: h }}>
        {loading ? (
          <div
            className="w-full animate-pulse bg-[#F0F4F8] rounded-lg"
            style={{ height: h }}
          />
        ) : hasData ? (
          <AgCharts
    ...
    <div className="bg-white border border-[#DDE5EF] rounded-2xl shadow-sm overflow-hidden animate-pulse">
      <div className="px-4 pt-4 pb-3 border-b border-[#F0F4F8]">
        <div className="flex gap-2 mb-2">
          <div className="h-4 w-20 rounded-full bg-[#EEF2F7]" />
          <div className="h-4 w-14 rounded-full bg-[#EEF2F7] ml-auto" />
        </div>
        <div className="h-4 w-3/4 rounded bg-[#EEF2F7] mb-1.5" />
        <div className="h-3 w-1/2 rounded bg-[#F3F6F9]" />
      </div>
      <div className="p-2">
        <div className="h-[200px] rounded-lg bg-[#F3F6F9]" />
      </div>
    </div>
  );
}
