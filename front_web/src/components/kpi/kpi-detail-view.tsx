...
import {
  ArrowsClockwiseIcon, FunnelSimpleIcon, XCircleIcon, CalendarBlankIcon,
...
export function KpiDetailView({ definition }: KpiDetailViewProps) {
  ...
  const { selectedKpi, kpiDetailLoading, kpiDetailError } =
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="px-5 pt-5 pb-4 border-b border-[#EEF2F7]">
        <div className="flex items-center gap-2 mb-2 flex-wrap">
          ...
          {filtered && (
            <span className="text-[10px] bg-[#ECFDF5] text-[#1E5B3C] px-2 py-0.5 rounded-full font-medium border border-[#BBF7D0]">
              Filtre actif
            </span>
          )}
        </div>
        ...
        {definition.unit && (
          <p className="text-[11px] text-[#9EB0C4] mt-1">
            Unité : <span className="font-medium text-[#7691A8]">{definition.unit}</span>
          </p>
        )}
      </div>
      ...
      <div className="px-5 py-3 border-b border-[#EEF2F7] bg-[#FAFBFC]">
        <div className="flex flex-wrap items-end gap-2">
          <CalendarBlankIcon size={13} className="text-[#9EB0C4] mb-2 flex-shrink-0" />
          <div className="flex flex-col gap-0.5">
            <label className="text-[9px] font-semibold text-[#B0BCC9] uppercase tracking-wide">Début</label>
            <input
              type="date"
              value={dateFrom}
              onChange={e => setDateFrom(e.target.value)}
              className="h-7 px-2 border border-[#DDE5EF] rounded-lg text-[11px] text-[#1B2633] bg-white focus:outline-none focus:border-[#1E5B3C]"
            />
          </div>
          <div className="flex flex-col gap-0.5">
            <label className="text-[9px] font-semibold text-[#B0BCC9] uppercase tracking-wide">Fin</label>
            <input
              type="date"
              value={dateTo}
          ...
          <button
            ...
            className="h-7 px-3 bg-[#1E5B3C] text-white text-[11px] font-semibold rounded-lg inline-flex items-center gap-1 hover:bg-[#174A30] transition-colors disabled:opacity-50"
          >
            {kpiDetailLoading
          ...
          {!filtered && (
            <button
              ...
              title="Recharger"
            >
              <ArrowsClockwiseIcon size={11} className={kpiDetailLoading ? 'animate-spin' : ''} />
            </button>
          )}
        </div>
      </div>
      ...
      <div className="flex-1 p-5 overflow-y-auto">
        ...
        {!isCurrentKpi && !kpiDetailLoading && !kpiDetailError && (
          <div className="flex flex-col items-center justify-center h-[300px] text-[#C5D0DC] gap-3">
            <svg width="40" height="40" fill="none" viewBox="0 0 24 24">
              ...
              <rect x="10" y="7" width="4" height="14" rx="1" fill="currentColor" opacity=".4" />
              <rect x="17" y="3" width="4" height="18" rx="1" fill="currentColor" opacity=".4" />
            </svg>
            <p className="text-[12px]">Cliquez sur Recharger pour afficher le graphique</p>
            <button
              onClick={() => load()}
              className="h-8 px-4 bg-[#1E5B3C] text-white text-[11px] font-semibold rounded-lg inline-flex items-center gap-1.5 hover:bg-[#174A30] transition-colors shadow-sm"
            >
        ...
        {isCurrentKpi && (
          <KpiChartCard
            kpi={{ ...selectedKpi!, label: definition.label, description: definition.description }}
        ...
        {!isCurrentKpi && kpiDetailLoading && (
          <div className="animate-pulse space-y-3">
            <div className="h-[380px] bg-[#F0F4F8] rounded-xl" />
          </div>
        )}
      </div>
    </div>
