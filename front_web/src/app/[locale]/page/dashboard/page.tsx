...
/* ── Finance DAF mock data ───────────────────────────────────────────── */

const FIN_KPI_DATA: FinKpi[] = [
...
const DAF_AGENT_ITEMS: AgentSyntheseItem[] = [
  ...
  { id: 3, model: "haiku",  type: "PRÉVISION TRÉSORERIE · 7 EXTRAITS", title: "Flux juillet 2026 · Projection automatique", desc: "Flux sortants estimés : 17,2M FCFA le 15 juil. (loyers + salaires). Solde projeté fin juillet : +12,4M FCFA. Vérifier avant diffusion direction.", meta: "Généré il y a 1h · photo relevé · Haiku 4.5" },
];

const DAF_AGENTS: AgentActif[] = [
...
const DAF_ALERTES: AlerteFinance[] = [
...
const DAF_CREANCES: CreanceClient[] = [
...
function FinanceDafSection() {
  ...
  return (
    <div className="mt-6 sm:mt-8">
      {/* Séparateur section DAF */}
      <div className="flex items-center gap-3 mb-4 sm:mb-6">
        <p className="text-[10px] font-bold uppercase tracking-[.1em] text-[var(--tx-3)] whitespace-nowrap">
          Direction Administrative &amp; Financière
        </p>
        <div className="flex-1 h-px bg-[var(--bd-def)]" />
        <button
          onClick={() => router.push(`/${locale}/page/finances/dashboard-daf`)}
          className="text-xs font-medium text-[var(--p500)] hover:underline flex items-center gap-1 whitespace-nowrap flex-shrink-0"
        >
          Vue complète <ArrowRightIcon size={12} />
        </button>
      </div>
      ...
      {/* KPI financiers */}
      <FinKpiRow kpis={FIN_KPI_DATA} />

      {/* Graphiques + Alertes + Créances */}
      <div className="grid grid-cols-1 xl:grid-cols-[1fr_320px] gap-3 sm:gap-4">
        ...
        <div className="flex flex-col gap-3 sm:gap-4">
          <AlertesFin alertes={DAF_ALERTES} />
          <CreancesTop creances={DAF_CREANCES} />
        </div>
      </div>
    </div>
  );
