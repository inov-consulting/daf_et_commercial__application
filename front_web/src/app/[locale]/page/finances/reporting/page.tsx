...
import {
  ...
  TrendUpIcon, TrendDownIcon, MinusIcon, ArrowClockwiseIcon,
...
export default function ReportingPage() {
  ...
  const lastCompletedRun = runs.find(r => r.status === 'completed' && r.summary);
  const snap = latestSnapshot;
  const subtitle = snap
    ? `${snap.period_label} · Groupe INOV Consulting`
    : 'Juin 2026 · Groupe INOV Consulting';
          ...
          <FinCard className="border-[rgba(16,185,129,.3)] bg-[rgba(16,185,129,.03)]">
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-2">
                ...
                <p className="text-[12px] font-semibold text-[var(--tx-1)]">Synthèse IA · Dernier run</p>
              </div>
              <button
                ...
                title="Actualiser"
              >
                <ArrowClockwiseIcon size={12} className={runsLoading ? 'animate-spin' : ''} />
              </button>
            </div>
            ...
            {runsLoading && !lastCompletedRun ? (
              ...
              <p className="text-[12px] text-[var(--tx-3)] italic">
                {runsError ? 'Erreur de chargement.' : 'Aucun run complété disponible.'}
              </p>
            )}
            ...
            {!runsLoading && runs.length > 0 && (
              <div className="mt-3 pt-2 border-t border-[rgba(16,185,129,.1)] space-y-1">
                <p className="text-[10px] font-bold uppercase tracking-widest text-[var(--tx-3)] mb-2">Runs récents</p>
                {runs.slice(0, 4).map(r => (
                  <div key={r.id} className="flex items-center justify-between gap-2">
                    <span className={cn(
                      'text-[10px] px-1.5 py-px rounded font-semibold',
                      r.status === 'completed' ? 'bg-[rgba(16,185,129,.1)] text-[#1B6B45]'
                        : r.status === 'running' ? 'bg-[rgba(99,102,241,.1)] text-[#4338CA]'
                        : r.status === 'failed'  ? 'bg-[rgba(239,68,68,.1)] text-[#DC2626]'
                        : 'bg-[var(--bg-sink)] text-[var(--tx-3)]'
                    )}>
                      {r.status}
                    </span>
                    <span className="text-[10px] text-[var(--tx-3)] font-mono truncate">{fmtDate(r.started_at)}</span>
                    <span className="text-[10px] text-[var(--tx-3)]">{r.proposed_actions_count} act.</span>
                  </div>
                ))}
              </div>
            )}
