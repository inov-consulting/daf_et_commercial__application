...
<body>{html_content}</body>
    ...
    async def generate_raw_content_into(
        self,
        cr: "CompteRenduOrm",
        content: str,
        extra_note_ids: list[UUID] | None = None,
        ...
        """Lance la génération Claude + PDF + MinIO sur un CR existant à partir de contenu brut.

        Équivalent de generate_into() mais prend du texte pré-formaté (messages WhatsApp,
        texte libre…) au lieu de notes DB. À appeler depuis une BackgroundTask.
        """
        import logging
        logger = logging.getLogger(__name__)
...
<body>{html_content}</body>
            ...
            slug = re.sub(r'[^\w\s-]', '', (contact_name or "whatsapp").lower())
            slug = re.sub(r'[-\s]+', '-', slug).strip('-')[:30] or "whatsapp"
            filename = f"CR-{slug}-v{cr.version}.pdf"
            folder = f"cr/whatsapp/{cr.parent_id}"
            minio_path = f"{folder}/{filename}"
            ...
            cr.status = "final"
            cr.prompt_used = prompt
            cr.note_ids = [str(nid) for nid in (extra_note_ids or [])]
            cr.generation_status = "done"
            cr.generation_error = None
            await cr.save()

            logger.info("cr.generate_raw_into.done cr_id=%s size=%d", cr.id, len(pdf_bytes))

        except Exception as exc:
            logger.exception("cr.generate_raw_into.failed cr_id=%s", cr.id)
            cr.generation_status = "failed"
            cr.generation_error = str(exc)[:1000]
            await cr.save()
